...
#
OLD_TRUST_RELATIONSHIP=$(aws iam get-role --role-name $SERVICE_ACCOUNT_IAM_ROLE \
...
#
SERVICE_ACCOUNT_IAM_POLICY_ID=$(aws iam get-policy \
...
#
SERVICE_ACCOUNT_IAM_ROLE_ARN=$(aws iam get-role --role-name \
  $SERVICE_ACCOUNT_IAM_ROLE --query 'Role.Arn' --output text)
...
then
  ...
  #
  aws iam attach-role-policy --role-name $SERVICE_ACCOUNT_IAM_ROLE \
    --policy-arn $SERVICE_ACCOUNT_IAM_POLICY_ARN  
else
  ...
  #
  aws iam update-assume-role-policy --role-name $SERVICE_ACCOUNT_IAM_ROLE \
